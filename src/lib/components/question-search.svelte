    ...
    {#if showResultsPanel}
      ...
      >
        ...
        {:else if response}
          ...
          >
            {#each response.results as result (result.questionId)}
              <article
                class="grid gap-3 border-b border-border/55 px-5 py-[1.125rem] [content-visibility:auto] [contain-intrinsic-size:auto_10rem] last:border-b-0 sm:grid-cols-[7.25rem_minmax(0,1fr)] sm:gap-6 sm:px-6"
              >
